...
struct MovieDetailsFactoryImp: MovieDetailsFactory {
    func makeModule(coordinator: MovieDetailsViewControllerCoordinator, movieItem: MovieItem) -> UIViewController {
        ...
        guard let movieId = String(movieItem.id) else {
            return UIViewController()
        }
        let movieDetailsRepository = MovieDetailsRepositoryImp(apiClientService: apiClientService, urlList: "\(Endpoint.baseMovieDetailsUrl.replacingOccurrences(of: "{movie_id}", with: ))")
